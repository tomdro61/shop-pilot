    ...
    <div>
      <div className="flex flex-wrap gap-2">
        {photos.map((photo) => (
          <div
            key={photo.id}
            className="relative h-16 w-16 rounded-md overflow-hidden bg-stone-100 dark:bg-stone-800"
          >
            {photo.signedUrl ? (
              <img
                src={photo.signedUrl}
        ...
        {canUpload && (
          <button
            ...
            className="flex h-16 w-16 items-center justify-center rounded-md border-2 border-dashed border-stone-200 dark:border-stone-700 text-stone-400 hover:border-blue-400 hover:text-blue-500 transition-colors disabled:opacity-50"
          >
            {isUploading ? (
