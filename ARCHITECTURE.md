...
- Staff-triggered send of a completed, **paid** job's itemized receipt — distinct from the invoice-to-pay and estimate-to-approve flows. Two surfaces: `SendReceiptButton` on the job detail page (shown on any `payment_status = 'paid'` job) and `QuickPayReceipt` on the Quick Pay success screen. Where the customer has no contact on file the operator types a destination; it is used for that send only and is **never** written to `customers`.
- The sender is `sendJobReceiptWith(supabase, params)` in `src/lib/receipts/send.ts` — a plain module, **deliberately not `"use server"`**: it takes an arbitrary destination and sends from the shop's verified domain, so an action id for it would be an open relay. It takes its Supabase client as an argument so the two entry points can supply different ones. Gates paid + `receipt_token` + non-null shop settings; each channel is isolated and reports its own status; the `messages` log is written OUTSIDE the send's try so a logging failure can never flip a delivered receipt to failed.
- Two entry points. **Managers:** `sendJobReceipt` (`src/lib/actions/receipts.ts`) — `requireManager()` + RLS client. **Staff incl. techs:** `POST /api/receipts/send` — `requireStaff()` + `createAdminClient()`, because RLS gives a tech no SELECT on a completed job (`record_quick_pay_job` writes `status='complete'` with no `assigned_tech`). Since RLS is no longer scoping, a tech must also pass the `paymentIntentId` of the payment they just took, matched against `jobs.stripe_payment_intent_id` and within 30 minutes of `paid_at` — job id alone would let any tech session pull an arbitrary historical receipt to a typed number, permanently, since `/receipt/[token]` is unauthenticated and never expires.
- Counter sales hang off the shared `WALK_IN_CUSTOMER_ID` row, whose name is literally "Walk-In Customer" and whose contact columns must stay null. Customer-facing output omits the name and the vehicle rather than substituting a placeholder (`paymentReceiptEmail` / `receiptSMS` take nullable name/vehicle; the receipt page heads with vehicle → job title → "Counter Sale").
...
- Auth via `requireCronSecret(request)` in `src/lib/cron/auth.ts` — checks `Authorization: Bearer ${CRON_SECRET}`. **`CRON_SECRET` is a manually-created Vercel env var; Vercel does NOT generate one.** Vercel only attaches the `Authorization` header when the var exists — with it unset, scheduled invocations arrive header-less. Extracted from the inline check once the second real route shipped.
...
- **Debugging a cron that "didn't run":** check the HTTP status in Vercel → Settings → Cron Jobs → View Logs *before* reading handler code. A cron rejected at the auth gate leaves no application-level trace — no `console.log`, no Sentry event — so a dead cron and a correctly-silent one look identical from inside the app. `requireCronSecret` fails closed on an unset `CRON_SECRET` and logs that misconfiguration separately from an invalid-token rejection, so this specific case is now visible in the runtime logs. (See PROGRESS.md Session 67 for the week-long silent outage this prevents.)
