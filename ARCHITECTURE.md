...
- **`CRON_SECRET`-unset lesson** (Session 67 — the actual root cause of the parking reminder never firing; Session 66's DST/Hobby fix above was real but was not why it was silent): `CRON_SECRET` had never been created in Vercel. `requireCronSecret` compared the header against a template literal, which with the var unset evaluates to the string `"Bearer undefined"`. Two consequences, both silent: every real Vercel cron invocation (header-less) 401'd before reaching any handler code — so no `console.log`, no Sentry event, no text, for *both* cron routes since the first one shipped — and any anonymous caller sending the literal `Bearer undefined` was authorized (verified against prod: `/api/cron/health` returned 200). This is why Session 66 could not find the `cron_health_ok` heartbeat and wrongly blamed Sentry queryability. Fix: `requireCronSecret` now fails closed on an unset secret and `console.error`s the misconfiguration, plus `CRON_SECRET` is set in all three Vercel environments. **A 401'd cron leaves no application-level trace — an auth gate that can't distinguish "misconfigured" from "intruder" makes a dead cron look like a clean one. Check Vercel → Cron Jobs → View Logs for the HTTP status before debugging handler logic.**
