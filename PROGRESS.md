...
- **Quick Pay jobs default to tax-off** (`charge_sales_tax: false` in the route insert). A Quick Pay is a flat, tax-inclusive counter amount, so there's nothing to tax on top; this keeps a later manager reconciliation (swap the placeholder for real parts + labor) summing to exactly what was collected instead of the 6.25%-on-parts toggle padding the total. No effect on un-reconciled jobs (the placeholder is a labor line → $0 tax either way). Normal jobs are unaffected (still default tax-on).
...
## Session 65 — 2026-07-16 — Parking prep reminder (7 PM nightly cron)
...
- `13554b5` `feat(parking): 7 PM nightly reminder for unprepped overnight-pickup cars` — committed on `staging`, merged to `master` (fast-forward `d32ab14..13554b5`). Post-deploy check passed: prod liveness green, new cron route returns 401 (auth-gated, confirms new code is live), health cron still 401 after the shared-auth refactor.
...
- **Cron count** — this brings Vercel to 3 cron entries. Hobby plan caps at 2; if on Hobby the extra entries may not register (Pro removes the cap). Existing health cron works, so crons are enabled — just confirm the count. If staying on Hobby, collapse the two DST entries to one daily entry (accepts a 1-hour winter drift) to stay under the cap.
- **First real fire:** 7 PM ET, production only. Can't be verified functionally until then (or a manual CRON_SECRET-bearer hit at 7 PM ET).
